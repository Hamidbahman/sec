...
namespace Authentication.Application
{
    public class OAuthService
    {
        ...
        public async Task<string?> GenerateAuthorizationCodeAsync(string clientId, string clientSecret, string? userCaptchaToken = null)
        {
            ...
            {
                return "Please wait before generation another code.";
            }
        ...
        await _redis.StringSetAsync($"rate_limit:{clientId}", DateTime.UtcNow.ToString(), TimeSpan.FromSecondes(30));

        string authCode = Convert.ToBase64String(RandomNumberGenerator.GetBytes(32));
        string encryptedAuthCode = encryptedAuthCode(authCode);
        _authCodes.TryAdd(authCode, clientId);

        await _redis.StringSetAsync($"auth_code:{encryptedAuthCode}", cliendId);
        //_authCodesExpiration[authCode] = DateTime.Now.AddMinutes(10); 
        return authCode;
        }
    private async bool IsRateLimited(string clientId)
    {
    ...
    {
        string encryptedAuthCode = encryptedAuthCode(authCode);
        return await _redis.StringGetAsync($"auth_code: {encryptedAuthCode}").IsNullOrEmpty;
    }
    ...
    {
        return storedHash == VerifyHashedSecret(inputSecret);
    }    

    private static string HashSecret(string secret)
    {
        byte[] salt = Encoding.UTF8.GetBytes("");
        return Convert.ToBase64String(KeyDerivation.pbkdf2(secret, salt, KeyDerivationPrf.HMACSHA256, 10000, 32));
    }
    ...
    public async Task<AuthResult> LoginAsync(string username, string password, string authenticationCode)
    {
        ...
        if (!BCrypt.Verify(password , user.UserProperty.Password))
        {
